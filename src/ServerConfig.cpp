...
void ServerConfig::setClientMaxBodySize(std::string parametr) {
  ...
  for (size_t i = 0; i < parametr.length(); i++) {
    if (parametr[i] < '0' || parametr[i] > '9')
      throw Error("Wrong syntax: client_max_body_size");
  }
  if (!ft_stoi(parametr))
    throw Error("Wrong syntax: client_max_body_size");
...
void ServerConfig::setErrorPages(std::vector<std::string> &parametr) {
  ...
  if (parametr.size() % 2 != 0)
    throw Error("Error page initialization faled");
...
void ServerConfig::setLocation(std::string path, std::vector<std::string> parametr) {
  ...
  for (size_t i = 0; i < parametr.size(); i++) {
    if (parametr[i] == "root" && (i + 1) < parametr.size()) {
      ...
      if (flag_max_size)
        throw Error("Maxbody_size of location is duplicated");
      checkToken(parametr[++i]);
  ...
  if (valid == 1)
    ...
    throw Error("Failed path in locaition validation");
  else if (valid == 3)
    throw Error("Failed redirection file in locaition validation");
  else if (valid == 4)
    throw Error("Failed alias file in locaition validation");
...
void ServerConfig::setupServer(void) {
  if ((_listen_fd = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
    LogService::printLog(RED, SUCCESS, "webserv: socket error %s   Closing ....", strerror(errno));
    exit(EXIT_FAILURE);
  }
  ...
  if (bind(_listen_fd, (struct sockaddr *)&_server_address, sizeof(_server_address)) == -1) {
    LogService::printLog(RED, SUCCESS, "webserv: bind error %s   Closing ....", strerror(errno));
    exit(EXIT_FAILURE);
  }
}